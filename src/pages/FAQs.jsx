    ...
    <div className=''>
      <div className='bg-[#F5F7FA] flex   justify-center items-center py-[2rem] gap-4 flex-col w-full'>
        ...
        <div>
          ...
          <span className='text-[#1D2026]'> FAQs</span>
        </div>
      </div>
        ...
        <ul className='sm:w-[400px] h-fit flex flex-col border border-[#E9EAF0]'>
          {categories.map((category, index) => (
            <li key={index} className='border-b border-[#E9EAF0]'>
              <button
                className={`w-full text-left px-4 py-3 hover:bg-[#FF6636] hover:text-white transition-all duration-200 ${activeCategoryIndex === index ? "bg-[#FF6636] text-white" : ""}`}
                onClick={() => {
            ...
            <div key={index} className='border border-[#E9EAF0] rounded-md'>
              <button
                ...
                className={`w-full flex justify-between items-center px-6 py-4 text-left font-semibold transition duration-300 ${
                  activeQuestionIndex === index ? "bg-[#1D2026] text-white" : "bg-white text-[#1D2026]"
                }`}
              >
                {item.question}
              ...
              {activeQuestionIndex === index && (
                <div className='px-6 pb-4 text-[#6E7485] transition duration-500'>
                  {item.answer}
                </div>
              )}
            </div>
     ...
     <div className='flex flex-col bg-[#F5F7FA] p-5 '>
        <h1 className='text-[#1D2026] text-[18px] font-[500]'>Didn’t find your answer?</h1>
        <p className='text-[#6E7485] text-[12px] font-[400]'>
          Don’t worry, write your question here and our support team will help you.
        </p>
        <form className='flex flex-col gap-4 py-5'>
          <input type="text" placeholder='Subject' className='px-4 py-2 bg-white focus:outline-none' />
          <textarea placeholder='Message' rows={5} className='px-4 py-2 bg-white focus:outline-none'></textarea>
